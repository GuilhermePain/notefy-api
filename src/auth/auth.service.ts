...
import { comparePasswords } from '../users/helpers/hash-password';
...
export class AuthService {
    ...
    async signIn(params: Prisma.UserCreateInput): Promise<{ access_token: string }> {
        const user = await this.userService.findUserEntityByEmail(params.email);
        if (!user) throw new NotFoundException("Email ou senha incorretos.");

        const passwordIsMatch = await comparePasswords(params.password, user.password);
